...
function getHotelInfo($aid,$show=1)
{
   ...
   $sql="select a.*,b.hotelrank from #@__hotel a left join #@__hotel_rank b on (a.hotelrankid=b.id and a.webid=b.webid) where  {$where} and a.webid='$webid'";
   $row=$dsql->GetOne($sql);
   return $row;
...
Class Hotel{
    ...
    public static function getHotelContent($row)
    {
        if($row['columnname']=='tupian')//图片模块特殊处理
        {
            ...
            {
                $cs = $i%4==0 ? "class='mr_0'" : '';
                $big = str_replace('lit160','allimg',$thumb);
                $out.='<li'.$cs.'><a  rel="group" href="'.$big.'"><img class="fl" src="'.$thumb.'" width="215" height="150" /></a></li>';
                $i++;
            }
            $out.='</ul>';
            return $out;
