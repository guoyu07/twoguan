   ...
   public static function addOrder($arr)
   {
	   ...
	   if(is_array($arr))
       {
           ...
           if($flag)
           {
               ...
               if($GLOBALS['cfg_supplier_msg_open']==1)
               {
                   ...
                   if(!empty($GLOBALS['cfg_webmaster_phone']))
                   {

                       Helper_Archive::sendMsg($GLOBALS['cfg_webmaster_phone'],'',$content);//发送短信.
                   }
                   //给供应商发送开启
                   if(!empty($GLOBALS['cfg_supplier_send_open']))
                   {
   ...
   /**
    ...
    public static function getPayTypeList($showxianxiadesc=1)
    {
       ...
       /* if(!empty($GLOBALS['cfg_alipay_account']) && !empty($GLOBALS['cfg_alipay_key']))
        ...
        $out = '';
      /*  if(strpos($GLOBALS['cfg_pay_type'],'1')!==false)
        {
            $out.="<input type='radio' name='choosepay' id='alipaytype' value='1' style='vertical-align:middle;margin-right:5px;'><label for='alipaytype'>支付宝</label>&nbsp;&nbsp;";
        }
        if(strpos($GLOBALS['cfg_pay_type'],'2')!==false)
        {
            $out.="<input type='radio' name='choosepay' id='billtype' value='2' style='vertical-align:middle;margin-right:5px;'><label for='billtype'>快钱</label>&nbsp;&nbsp;";
        }
        if(strpos($GLOBALS['cfg_pay_type'],'3')!==false)
        {
            $out.="<input type='radio' name='choosepay' id='huicaotype' value='3' style='vertical-align:middle;margin-right:5px;'><label for='huicaotype'>网银支付</label>&nbsp;&nbsp;";
        }
        if(strpos($GLOBALS['cfg_pay_type'],'4')!==false)
        {
            $out.="<input type='radio' name='choosepay' id='yltype' value='4' style='vertical-align:middle;margin-right:5px;'><label for='yltype'>银联支付</label>&nbsp;&nbsp;";
        }
        if(strpos($GLOBALS['cfg_pay_type'],'5')!==false)
        {
            $out.="<input type='radio' name='choosepay' id='qbtype' value='5' style='vertical-align:middle;margin-right:5px;'><label for='yltype'>钱包支付</label>&nbsp;&nbsp;";
        }*/
        //第三版
        $out = '<dl>
            	<dt>在线支付方式</dt>
        ...
        if(strpos($GLOBALS['cfg_pay_type'],'1')!==false)
        {
            ...
            $out.='  <li>
                        ...
                        <span>支付宝</span>
                        <input type="radio" name="choosepay" value="1" data-type="支付宝">
                      </label>
                    </li>';
        }
        if(strpos($GLOBALS['cfg_pay_type'],'2')!==false)
        ...
        if(strpos($GLOBALS['cfg_pay_type'],'7')!==false)
        {
            $out.='<li>
                    	<label>
        ...
        $out.='</ul></dd></dl>';
        if(strpos($GLOBALS['cfg_pay_type'],'6')!==false)
        {
    ...
    {
        global $dsql;
        $sql = "select min(price) as price from #@__hotel_room_price where hotelid='$hotelid' and price!=0";
        $row = $dsql->GetOne($sql);
        return $row['price'];
    }
