...

{
   ...
   $picwhere2=!empty($haspic)?" where (a.litpic is not null and a.litpic!='') and a.ishidden=0 ":'';
	...
	else if($flag=='relative')
	{
		...
		$where=" FIND_IN_SET($maxkindid,a.kindlist) and a.ishidden=0";
	    //排序顺序：置顶+tag关联》排序+ tag关联》最新更新+tag关联
	    $sql="select a.* from #@__article a where  {$where} order by a.modtime desc,a.addtime desc limit {$limit},{$row} ";
