...
function lib_getjieban(&$ctag,&$refObj)
{
    ...
    foreach($arr as $row)
    {
        ...
        if($flag!='join')
        {
            ...
            $row['kindnamelist'] = TagJB::getKindnameList($row['kindlist'],'-');
        ...
        else
        {
            ...
            $row['litpic'] = $memberinfo['litpic'] ? $memberinfo['litpic'] : getDefaultImage();
...
class TagJB{
  ...
  * */
    public static function getJiebanTitle($arr)
    {
        ...
        $dest = self::getKindnameList($arr['kindlist']);
        $dest = explode(',',$dest);
        $dest = implode('--',$dest);
        return $dest.$arr['day'].'日游行程';
    }
