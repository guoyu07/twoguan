	...
	if(!empty($flag))//首页调用其它栏目的中间导航
	{
		...
		if(!empty($pid))
		  $w=" and a.pid=$pid";
		   ...
		   if($isnav==1){
		   	$sql="select a.kindname,b.kindid,b.shownum,a.jieshao,a.pinyin,a.litpic,a.pinyin from  #@__destinations as a inner join {$tablename[$typeid]} as b on a.id=b.kindid where b.isnav=1 and a.isopen=1 $w order by b.displayorder asc limit $limit,$row";
		   }else if($ishot==1){
		   	$sql="select a.kindname,b.kindid,b.shownum,a.jieshao,a.pinyin,a.litpic,a.pinyin from  #@__destinations as a inner join {$tablename[$typeid]} as b on a.id=b.kindid where b.ishot=1 and a.isopen=1 $w order by b.displayorder asc limit $limit,$row";
		   }else{
		   	$sql="select a.kindname,b.kindid,b.shownum,a.jieshao,a.pinyin,a.litpic,a.pinyin from  #@__destinations as a inner join {$tablename[$typeid]} as b on a.id=b.kindid where a.isopen=1 $w order by b.displayorder asc limit $limit,$row";
		   }
