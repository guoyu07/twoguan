   ...
   if($User->IsLogin())
		{
			...
			// $out.='</span>';
             ...
             $out.='  <a class="mb-a" href="'. $GLOBALS['cfg_base_url'].'/member/">我的订单</a>
                <div class="mb-hide">
                    <a class="bk_ico1" href="'. $GLOBALS['cfg_base_url'].'/member/?chooseitem=unpay"><s></s>待付款</a>
                    <a class="bk_ico2" href="' . $GLOBALS['cfg_base_url'] . '/member/"><s></s>订单中心</a>
                    <a class="bk_ico3" href="' . $GLOBALS['cfg_base_url'] . '/member/?dopost=userinfo"><s></s>帐号资料</a>
                </div>';
             $out.='</span>';
            $out.='<a href="'.$GLOBALS['cfg_base_url'].'/member/query.php">订单查询</a>';
            $out.='<a href="'.$GLOBALS['cfg_base_url'].'/member/login.php?dopost=logout">退出</a>';
			
		}
		else //未登陆状态
		{
			
          $out = '<a href="' . $GLOBALS['cfg_base_url'] . '/member/login.php">登录</a>' .
			       '<a href="' . $GLOBALS['cfg_base_url'] . '/member/reg.php">注册</a>';
          $out.='<a href="'.$GLOBALS['cfg_base_url'].'/member/query.php">订单查询</a>';
