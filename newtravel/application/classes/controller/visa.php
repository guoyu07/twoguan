...
class Controller_Visa extends Stourweb_Controller{
	...
	public function action_visa()
	{
		...
		{
           $this->assign('kindmenu',json_encode(Common::getConfig('menu_sub.visakind')));//分类设置项
		...
		else if($action=='read')    //读取列表
		{
			...
			$sql="select a.*,b.displayorder from sline_visa as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=8)  where $w $order limit $start,$limit";
            //echo $sql;
