...
class Controller_Tongyong  extends Stourweb_Controller{
	...
	public function action_index()
	{
		...
		if(empty($action))  //显示列表页
		{
           ...
           $this->assign('kindmenu',json_encode(self::$kind));//分类设置项
		...
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			   $sql="select a.*,b.displayorder from sline_model_archive as a left join sline_allorderlist b on (a.id=b.aid and b.typeid='$typeid') where $w $order";
			}
			else
			{
			   $sql="select a.*,b.displayorder from sline_model_archive as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid='$typeid')  where $w $order ";
