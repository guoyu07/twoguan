...
class Controller_Index extends Stourweb_Controller{
    ...
    public function action_ajax_order_num()
    {
        ...
        foreach($arr as $row)
        {
            ...
            $sql = "select count(*) as num from sline_member_order where addtime>=$starttime and addtime<=$endtime and typeid='".$row['id']."'";
            $ar = DB::query(1,$sql)->execute()->as_array();
            $count = $ar[0]['num'];
            $out[]=array('md'=>$row['pinyin'],'num'=>$count);
        }
        //自定义订单
        $count = 0;
        $sql = "select count(*) as num from sline_dzorder where addtime>=$starttime and addtime<=$endtime";
        $row = DB::query(1,$sql)->execute()->as_array();
        $count = $row[0]['num'];
        $out[]=array('md'=>'zdy','num'=>$count);

        //私人定制定单
        $count = 0;
        $sql = "select count(*) as num from sline_customize where addtime>=$starttime and addtime<=$endtime";
        $row = DB::query(1,$sql)->execute()->as_array();
        $count = $row[0]['num'];
        $out[]=array('md'=>'custom','num'=>$count);
