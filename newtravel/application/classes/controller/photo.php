...
class Controller_Photo  extends Stourweb_Controller{
	...
	public function action_photo()
	{
		...
		{
           $this->assign('kindmenu',json_encode(Common::getConfig('menu_sub.photokind')));//分类设置项
		...
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			   $sql="select a.id,a.aid,a.photoname,a.litpic,a.headimgid,a.alt,a.title,a.kindlist,a.attrid,a.webid,a.photokind,a.themelist,a.addtime,a.modtime,a.ishidden,b.isjian,b.displayorder,b.isding from sline_photo as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=6) where $w $order limit $start,$limit";
			}
			else
			{
			   $sql="select a.id,a.aid,a.photoname,a.litpic,a.headimgid,a.alt,a.title,a.kindlist,a.attrid,a.webid,a.themelist,a.photokind,a.addtime,a.modtime,a.ishidden,b.isjian,b.displayorder,b.isding from sline_photo as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=6) where $w $order limit $start,$limit";
