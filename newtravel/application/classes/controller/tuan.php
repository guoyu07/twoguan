...
class Controller_Tuan extends Stourweb_Controller{
	...
	public function action_tuan()
	{
		...
		{
           $this->assign('kindmenu',json_encode(Common::getConfig('menu_sub.tuankind')));//分类设置项
		...
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			  $sql="select a.id,a.webid,a.aid,a.title,a.shownum,a.bookcount,a.totalnum,a.virtualnum,a.validdate,a.attrid,a.kindlist,a.jifenbook,a.jifentprice,a.jifencomment,a.iconlist,a.themelist,b.isjian,b.isding,ifnull(b.displayorder,9999) as displayorder,ishidden from sline_tuan as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=13)  where $w $order limit $start,$limit";
			}
			else
			{
			    $sql="select a.id,a.webid,a.aid,a.title,a.shownum,a.bookcount,a.totalnum,a.virtualnum,a.validdate,a.attrid,a.kindlist,a.jifenbook,a.jifentprice,a.jifencomment,a.iconlist,a.themelist,b.isjian,b.isding,ifnull(b.displayorder,9999) as displayorder,ishidden from sline_tuan as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=13)  where $w $order limit $start,$limit";
			}
