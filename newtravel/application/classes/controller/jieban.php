...
class Controller_Jieban extends Stourweb_Controller{
	...
	public function action_index()
	{
		...
		{
           $this->assign('kindmenu',json_encode(Common::getConfig('menu_sub.jiebankind')));//分类设置项
		...
		else if($action=='read')    //读取列表
		{
			...
			$sql="select a.*,ifnull(b.displayorder,9999) as displayorder from sline_jieban as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=$typeid)  where $w $order limit $start,$limit";
			...
			foreach($list as $k=>$v)
			{
                ...
                $v['title']= Model_Jieban::genTitle($v['kindlist'],$v['day']);
