    ...
    public function action_ajax_getfooternav()
    {
        ...
        foreach($arr as $row)
        {
            ...
            $tr ='
                  ...
                  <td align="center" onclick="edit('.$row['id'].')">'.Common::getIco('edit').'</td>
                  <td align="center" onclick="del('.$row['id'].',this)">'.Common::getIco('del').'<input type="hidden" name="id[]" value="'.$row['id'].'"/></td>
                </tr>
            ';
            array_push($out,$tr);
        }
