...
class Controller_Car  extends Stourweb_Controller{
	...
	public function action_car()
	{
		...
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			   $sql="select  a.id,a.webid,a.aid,a.carname,a.seatnum,a.carkindid,a.carbrandid,a.kindlist,a.attrid,a.iconlist,a.themelist,a.ishidden,c.kindname as brandname,d.kindname as carkindname from sline_car as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=3) left join sline_car_brand c on (a.carbrandid=c.id and a.webid=0) left join sline_car_kind d on (a.carkindid=d.id) where $w $order limit $start,$limit";
			}
			else
			{
				$sql="select  a.id,a.webid,a.aid,a.carname,a.seatnum,a.carkindid,a.carbrandid,a.kindlist,a.attrid,a.iconlist,a.themelist,a.ishidden,b.isjian,b.isding,b.istejia,ifnull(b.displayorder,9999) as displayorder ,c.kindname as brandname,d.kindname as carkindname from sline_car as a left join sline_kindorderlist as b on (a.id=b.id and b.classid=$kindid and b.typeid=3) left join sline_car_brand c on (a.carbrandid=c.id) left join sline_car_kind d on (a.carkindid=d.id)  where FIND_IN_SET($kindid,a.kindlist)   order by a.modtime desc";
			}
