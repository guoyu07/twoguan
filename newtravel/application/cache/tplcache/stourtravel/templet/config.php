...
<head>
    ...
    <?php echo Common::getScript('jquery.jqtransform.js,jquery.upload.js');?>
</head>
<style>
    .templetbox{
...
<body style="background-color: #fff">
    <div class="out-box-con">
      ...
      <div id="templet_list">
          ...
          <?php $n=1; if(is_array($templet_list)) { foreach($templet_list as $v) { ?>
            <div class="onedata">
              <div class="v1">
                  <?php echo $v['path'];?>
              </div>
              <div class="v2">
                  <a href="javascript:;" data-path="<?php echo $v['path'];?>" ><?php echo Common::getIco('edit');?></a>
              </div>
              <div class="v3">
                  <input name="isuse" type="radio" data-id="<?php echo $v['id'];?>" value="<?php echo $v['isuse'];?>" <?php if($v['isuse']==1) { ?>checked="checked"<?php } ?>
/>
              </div>
            </div>
          <?php $n++;}unset($n); } ?>
   ...
   <div style="width:563px; height:30px;float:left ">
       <div style="width:559px; height:30px; margin-left:2px; padding-top: 20px;">
           <a href="#" style="display:block; width:80px; height:22px; float:left; margin:4px 0px">
               <img src="<?php echo $GLOBALS['cfg_public_url'];?>images/shangchuan.png" style="border:0;" onclick="uploadTemplet()" />
           </a>
           ...
           <a href="#" style="display:block; width:110px; height:30px; float:right;">
               <img src="<?php echo $GLOBALS['cfg_public_url'];?>images/shangcheng.png" style="border:0;" />
           </a>
       </div>
   </div>
<script language="JavaScript">
    ...
    function addEvent()
    {
        ...
        $(".v2").find('a').unbind('click').click(function(){
    ...
    function uploadTemplet()
    {
            ...
            $.upload({
                ...
                onComplate: function(data) {
                    ...
                    //如果上传成功
                    if(data.status){
                        var html='<div class="onedata">'
                            html+='<div class="v1">'+data.path+'</div>';
                            html+='<div class="v2"><a href="javascript:;" data-path="'+data.path+'" >'+editico+'</a></div>';
