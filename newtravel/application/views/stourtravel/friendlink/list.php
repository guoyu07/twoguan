            ...
            Ext.create("Ext.window.Window",{
                ...
                buttons: {
                    ...
                    items: [
                        { xtype: 'button', text: '提交', handler: function (btn) {
                            var pdom=btn.up('window').getEl();
                            var sitename_dom=pdom.down('.sitename');
                            var siteurl_dom=pdom.down('.urladdress');
                            var webid=pdom.down('.sitewebid').getValue();

                            var sitename=sitename_dom.getValue();
                            var siteurl=siteurl_dom.getValue();
                            ...
                            Ext.Ajax.request({
                                ...
                                method  :  "POST",
                                datatype  :  "JSON",
                                params:{sitename:sitename,siteurl:siteurl,webid:webid},
                ...
                listeners:{
                    boxready:function(win){
                        ...
                        var params=window.product_store.getProxy().extraParams;
                        var webid=params.webid?params.webid:0;
                        var web_select="<select name='webid' class='sitewebid'>"
                        Ext.Array.each(window.WEBLIST,function(row,index,itself){
                            var is_selected=webid==row.webid?"selected='selected'":'';
                            web_select+="<option value='"+row.webid+"' "+is_selected+">"+row.webname+"</option>";
                        });
                        web_select+="</select>";
                        html="<table class='yqlj-set-tab'><tr><td width='60'>网站名称: </td><td><input type='text' size='20' class='sitename'/><font color='red'>*</font></td></tr>";
                        html+="<tr><td>选择站点: </td><td>"+web_select+"</td></tr>";
                        html+="<tr><td>URL地址: </td><td><input type='text' size='30' class='urladdress'/><font color='red'>*</font></td></tr></table>"
                        win.update(html);
	  ...
	  window.product_grid=Ext.create('Ext.grid.Panel',{ 
	   ...
	   columns:[
			   ...
			   },
			   {
				   ...
				   dataIndex:'siteurl',
				   renderer : function(value, metadata,record) {    
						   return "<a href='javascript:;' onclick=\"window.open('"+value+"','_blank')\">查看</a>";
	           ...
	           { 
				 ...
				 listeners:{
					  boxready:function(win)
					  {
						  ...
						  var html="<div><span style=\"margin-left:10px\"><input onclick=\"choosePos(this,9,-1)\" type=\"checkbox\"  value=\"-1\" style=\"vertical-align:middle\">全选</span>";
