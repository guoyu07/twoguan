...
if($dopost=='loginbyweixi')
{
	 ...
	 $state = time();
	 $weixi=new weixinPHP($appid,$appkey,$callback_url,$state);
	 ...
	 else
	 {
		 ...
		 if(!empty($openid))
		 {
			 $sql="select * from #@__member where `connectid`='$openid' and `from`='weixi'";
			 ...
			 {
			   //未存在于数据库中，跳转到注册绑定页面绑定用户.介于腾讯的霸王条款,绑定注册页面禁用.
			    $user=$weixi->get_user_info($openid,$token);
				...
				$url="{$GLOBALS['cfg_basehost']}/member/login.php?dopost=savebind&uname=".urlencode($user['nickname']);
				$url.="&pwd=123456789&connectid=".$openid.'&from=weixi';
				header("Location:$url");
			    exit;
