...
if($action=='step5_getproductlist')
{
    ...
    if(!empty($destid))
    {
        ...
        $sql = "select a.id,a.aid,a.linename,a.linepic as litpic,sellpoint from sline_line a left join #@__kindorderlist c on (c.classid=$destid and a.id=c.aid  and c.typeid=1) $where $orderby  limit 0,3";
        $arr = $dsql->getAll($sql);
    ...
    else if(!empty($userplace))
    {
        ...
        if(!empty($uday))$where.=" and lineday='$uday'";
        $orderby="order by case when c.displayorder is null then 9999 end,c.displayorder asc,a.modtime desc,a.addtime desc";
        $sql = "select a.id,a.aid,a.linename,a.linepic as litpic,sellpoint from sline_line a left join #@__allorderlist c on (a.id=c.aid  and c.typeid=1) $where $orderby  limit 0,3";
        $arr = $dsql->getAll($sql);
    }
    ...
    if(!empty($arr))
    {
        ...
        {
            $arr[$key]['url'] = $GLOBALS['cfg_basehost'].'/lines/show_'.$row['aid'].'.html';
            $arr[$key]['title'] = $row['linename'];
            $arr[$key]['price'] = ULine::getMinTprice($row['id']);
...
if($action == 'addjoin')
{
    $status = 0 ;
    //手机号码验证
    $preg = '/13[1-9]{1}\d{8}|15[1-9]\d{8}|188\d{8}/';
