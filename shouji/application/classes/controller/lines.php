...
class Controller_Lines extends Stourweb_Controller{
	...
	public function action_index(){
		...
		if(!empty($data)){
            foreach ($data as $key => $value) {
				$temsql = $sql="select a.id,a.linename,a.lineprice,a.linepic,IFNULL(b.displayorder,9999) as displayorder from sline_line as a left join sline_kindorderlist b on (a.id=b.aid and b.typeid=1 and b.classid=".$value[id].") where find_in_set(".$value['id'].",a.kindlist)  order by displayorder asc limit 0,4";
	...
	public function action_show()
	{
		...
		//如果行程类型为2
		if($row['isstyle']=='2'){
			$temjeishao = ORM::factory('line_jieshao')->where("lineid=".$row['id'])->order_by("day",'asc')->get_all();
		...
		$row['transport'] = trim($transportstr,',');
